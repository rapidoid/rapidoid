...
public class BeanParamRetriever extends RapidoidThing implements ParamRetriever {
    ...
    public Object getParamValue(Req req) {
        ...
        if (validate) {
            BeanValidator validator = customization.validator();

            U.must(validator != null, "The @Valid annotation was used on %s, but bean validator was not configured!", type);

            String errMsg = validator.validate(req, bean);

            if (U.notEmpty(errMsg)) {
                throw new RapidoidValidationError(errMsg);
            }
        }
