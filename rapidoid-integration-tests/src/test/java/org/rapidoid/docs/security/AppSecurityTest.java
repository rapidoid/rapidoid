...
@Doc(title = "Role-based security")
public class AppSecurityTest extends DocTest {
	...
	protected void exercise() {
		...
		GET("/manage");
		GET("/hi");
		POST("/_login", U.map("username", "foo", "password", "wrong"));
		POST("/_login", U.map("username", "foo", "password", "foo"));
		GET("/manage");
		GET("/hi");
	}
