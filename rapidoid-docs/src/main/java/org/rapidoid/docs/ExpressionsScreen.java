...
public class ExpressionsScreen extends Screen {
	...
	Object content() {
		...
		expr("%LINE%", btn("abc"));
		expr("%LINE%", btnPrimary("abc"));
		expr("%LINE%", btnSuccess("abc"));
		expr("%LINE%", btnInfo("abc"));
		expr("%LINE%", btnWarning("abc"));
		expr("%LINE%", btnDanger("abc"));
		...
		expr("%LINE%", row(col6("g"), col5("g"), col1("h")));
		expr("%LINE%", mid2("ab"));
